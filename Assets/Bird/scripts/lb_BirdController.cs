...
public class lb_BirdController : MonoBehaviour {
    ...
    public float fleeDistance = 20f; // 새가 플레이어로부터 멀어질 거리

    private bool isNearBy = false;
    private Transform playerTransform; // 플레이어의 Transform 컴포넌트   

    private bool isFlyingAway = false;    // 튕겨나가는 동작 중인지 여부
    private Vector3 flyAwayTarget;        // 새가 날아갈 위치

    private Rigidbody rb;
	private lb_Bird bird_ct;
    ...
    private void Update()
    {
		...
		for (int i = 0; i < myBirds.Length; i++)
		{
			if (Vector3.Distance(myBirds[i].transform.position, playerTransform.position) < detectionRange)
			{
				//myBirds[i].SendMessage("UpdateTargets");
				//myBirds[i].SendMessage("Flyaway");
				//StopCoroutine("UpdateTargets");
				//StartCoroutine("UpdateTargets");
				//BirdFindTarget(myBirds[i]);
				
				rb = myBirds[i].GetComponent<Rigidbody>();

                Vector3 force = new Vector3(0, 0.2f, 0); // y축 방향으로 10의 크기를 가진 힘

                rb.AddForce(force, ForceMode.Impulse);
				if (myBirds[i].activeSelf)
				{
					myBirds[i].SendMessageUpwards("FlyAway");
                }

                print("새 가까워서 날아가는거 작동함");
			}
		}
    }
}
